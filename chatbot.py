...
data_file = open('/Users/jalilkhan/Downloads/chatbot/PredefinedPatternsResponses.json').read()
intents = json.loads(data_file)


# ## Preprocessing Data
# 
# Tokenizing is  first thing to do on text data. 
# Its process of breaking the whole text into small parts like words.
# 

# In[6]:


# iterate through the patterns and tokenize the sentence using nltk.word_tokenize() function
...
# ## Creatingtraining and testing data
# 
...
# ## Building the model
# 
# Now after having our training data ready we now build neural network with 3 layers by using keras squential API for this. we achieve 100 percent accuracy after training the model for 200 epochs.
...
# ## Now for pridicting the responce from the bot we will load trained model. 
# The model will only tell us the class it belongs to and then retrieve a response from the list of the responses.

# In[11]:


#  ‘words.pkl’ and ‘classes.pkl’ pickle files which we have created when we trained our model
...
import json
import random
intents = json.loads(open('/Users/jalilkhan/Downloads/chatbot/PredefinedPatternsResponses.json').read())
